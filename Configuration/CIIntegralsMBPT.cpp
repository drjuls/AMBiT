    ...
    {   pOrbitalConst s2 = it_2.GetState();
        ...
        {   pOrbitalConst s4 = it_4.GetState();
            ...
            while(k <= kmax)
            {
                // s1 is the smallest
                it_1.First(); i1 = 0;
                while((i1 <= i2) && (i1 <= i4) && (it_1.GetState()->PQN() <= max_pqn_1))
                {   pOrbitalConst s1 = it_1.GetState();
                    ...
                    {   pOrbitalConst s3 = it_3.GetState();
                           ...
                           && (double(k) <= s1->J() + s3->J()))
                        {
                            ...
                            unsigned int smallest_pqn = s2->PQN();
                            unsigned int second_smallest_pqn = s3->PQN();
                            if(smallest_pqn > second_smallest_pqn)
                                swap(smallest_pqn, second_smallest_pqn);
                            if(s4->PQN() < smallest_pqn)
                            {   second_smallest_pqn = smallest_pqn;
                                smallest_pqn = s4->PQN();
                            }
                            else if(s4->PQN() < second_smallest_pqn)
                                second_smallest_pqn = s4->PQN();
        ...
        {   pOrbitalConst s2 = it_2.GetState();
            ...
            {   pOrbitalConst s4 = it_4.GetState();
                ...
                while(k <= kmax)
                {
                    // s1 is the smallest
                    it_1.First(); i1 = 0;
                    while((i1 <= i2) && (i1 <= i4) && (it_1.GetState()->PQN() <= box_max_pqn_1))
                    {   pOrbitalConst s1 = it_1.GetState();
...
void CIIntegralsMBPT::UpdateOneElectronIntegrals(const std::string& sigma_id)
{
    ...
    while(!it_i.AtEnd())
    {
        ...
        while(!it_j.AtEnd())
        {
            ...
            // Overlap integrals
            const Orbital& p1 = *it_i.GetState();
            const Orbital& p2 = *it_j.GetState();
            if(p1.L() == p2.L())
            {
                double overlap = 0.;
                           ...
                           && (double(k) <= s1->J() + s3->J()))
                        {
                               ...
                               (second_smallest_pqn <= max_pqn_3))
                            {
                                ...
                                if(TwoElectronIntegrals.find(key) == TwoElectronIntegrals.end())
                                {
                                    ...
                                    for(p=0; p<limit; p++)
                                    {
                                        radial += (s1->f[p] * s3->f[p] + s1->g[p] * s3->g[p])
                                                * Pot24[p] * dR[p];
                                    }

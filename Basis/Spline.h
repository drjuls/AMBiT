 ...
 *          n = 1..nderiv
 */
void bsplvd(const double* t, int* k, double* x, int* left, double* dbiatx, int* nderiv);

/** Function returns the value of the (jderiv)th derivative of
 *      f(x) = Sum[bcoef(i) * BSpline(i, x), i=1..n]
 *  at the point x.
 */
double bvalue(const double* t, const double* bcoef, int* n, int* k, double* x, int* jderiv);

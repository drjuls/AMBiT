...
differ between MKL versions, but it's usually one of either `-DMKL_INTERFACE_FULL=lp64` or
`-DMKL_INTERFACE_FULL=gf_lp64`. If `MKL_INTERFACE_FULL` is not explicitly set, then CMake will
...
varies between MKL versions, but it's usually one of either `-DMKL_INTERFACE_FULL=lp64` or

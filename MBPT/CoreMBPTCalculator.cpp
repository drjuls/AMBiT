...
unsigned int CoreMBPTCalculator::GetStorageSize(const ExcitedStates* valence_states)
{
    if(!integrals)
...
void CoreMBPTCalculator::UpdateIntegrals(const ExcitedStates* valence_states)
{
    SetValenceEnergies();
    if(integrals)
        delete integrals;
                ...
                if(C_nalpha)
                {
                    ...
                    while(!it_beta.AtEnd())
                    {
                        ...
                        if(coeff)
                        {
                            ...
                            coeff = coeff/(ValenceEnergy + sn.GetEnergy() - sbeta.GetEnergy() - salpha.GetEnergy() + delta);
                    ...
                    while(!it_m.AtEnd())
                    {
                        ...
                        if(coeff)
                        {
                            ...
                            coeff = coeff/(ValenceEnergy + salpha.GetEnergy() - sn.GetEnergy() - sm.GetEnergy() - delta);
...
void CoreMBPTCalculator::CalculateCorrelation2(int kappa, SigmaPotential* sigma) const
{
    const bool debug = DebugOptions.LogMBPT();
    unsigned int MaxStateSize = core->GetConstHFPotential().size();
    const double NuclearInverseMass = core->GetNuclearInverseMass();
                ...
                if(C_nalpha)
                {
                    ...
                    while(!it_beta.AtEnd())
                    {
                        ...
                        if(C_abeta && ((sa.L() + salpha.L())%2 == (sn.L() + sbeta.L())%2))
                        {
                            ...
                            C_abeta = C_abeta/(ValenceEnergy + sn.GetEnergy() - sbeta.GetEnergy() - salpha.GetEnergy() + delta);
...
void CoreMBPTCalculator::CalculateCorrelation4(int kappa, SigmaPotential* sigma) const
{
    const bool debug = DebugOptions.LogMBPT();
    unsigned int MaxStateSize = core->GetConstHFPotential().size();
    const double NuclearInverseMass = core->GetNuclearInverseMass();
                ...
                if(C_nalpha)
                {
                    ...
                    while(!it_m.AtEnd())
                    {
                        ...
                        if(C_am && ((sa.L() + sn.L())%2 == (sm.L() + salpha.L())%2))
                        {
                            ...
                            C_am = C_am/(ValenceEnergy + salpha.GetEnergy() - sn.GetEnergy() - sm.GetEnergy() - delta);
...
double CoreMBPTCalculator::CalculateCorrelation1and3(const OrbitalInfo& sa, const OrbitalInfo& sb) const
{
    ...
    while(!it_n.AtEnd())
    {
        ...
        const double En = it_n.GetState()->GetEnergy();

        ConstStateIterator it_alpha = excited->GetConstStateIterator();
        while(!it_alpha.AtEnd())
        {
            const OrbitalInfo salpha = it_alpha.GetOrbitalInfo();
            const double Ealpha = it_alpha.GetState()->GetEnergy();
            ...
            while(k1 <= k1max)
            {
                ...
                if(C_nalpha)
                {
                    ...
                    while(!it_beta.AtEnd())
                    {
                        ...
                        const double Ebeta = it_beta.GetState()->GetEnergy();
                    ...
                    while(!it_m.AtEnd())
                    {
                        ...
                        const double Em = it_m.GetState()->GetEnergy();

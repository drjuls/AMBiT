...
def configure_environment(env, conf):
    """ Configures the supplied compilation environment according to the configuration options in conf.
        ...
        conf - Dict generated by parsing the YAML in the configuration file (config.yml)
    """
    build_info = conf["Build options"]
    ...
    eigen_dir = build_info["Eigen path"]
    sparsehash_dir = build_info["Sparsehash path"]
    ...
    for lib in [l for l in libs if l.find("boost") == -1]:
        if not env_conf.CheckLib(lib): # Can't find in current path
            if(not pkgconfig_exists or not env_conf.check_pkg(lib)):
                print("Warning: could not find library {}. Aborting".format(lib))
                exit(-1)
            else:
    ...
    if not build_info["Sparsehash path"]:
        ...
        if env_conf.check_pkg("libsparsehash"):
            ...
            print("Failed to automatically locate Eigen headers. Please specify Sparsehash path in config.yml")
            exit(-1)
    if not build_info["Eigen path"]:
        print("Eigen directory not specified...")
        if env_conf.check_pkg("eigen3"):
            env_conf.env.ParseConfig("pkg-config --libs --cflags eigen3")
        else:
            print("Failed to automatically locate Eigen headers. Please specify Eigen path in config.yml")
            exit(-1)
...
if not env.GetOption('clean'):
    env = configure_environment(env, conf)

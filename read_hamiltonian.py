...
# TODO: Talk to Julian about whether the upper or lower triangular matrix is stored to file.
# TODO: Someone who shall remain nameless (they start with a J and ends with an ulian) changed which
#       fucking triangle AMBiT stores in the middle of February. Double check this is actually stable
...
if __name__ == '__main__':
  #ham7f = HamiltonianMatrix("artificial-inject/7f_0.8o.matrix")
  #ham8f = HamiltonianMatrix("artificial-inject/8f_0.8o.matrix")
  inject = HamiltonianMatrix("artificial-inject/20f_0.8o.matrix")
  noInject = HamiltonianMatrix("artificial-no-inject/20f_0.8o.matrix")

  print("\n20f - Inject orbitals:\n===\n")
  print("Diagonal:\n{}".format(inject.H.diagonal()))

  print("Large matrix elements:")
  #for ii in range(14, 17):
  for ii in range(inject.N):
    for jj in range(inject.N):
      if(abs(inject.H[ii, jj]) >= 0.4):
        print("H[{}][{}] = {}".format(ii, jj, inject.H[ii, jj]))
  print("\n")
  
  print("20f - No inject orbitals:\n===\n")
  print("Diagonal:\n{}".format(noInject.H.diagonal()))
  
  print("Large matrix elements:")
  #for ii in range(14, 17):
  for ii in range(noInject.N):
    for jj in range(noInject.N):
      if(abs(noInject.H[ii, jj]) >= 0.4):
        print("H[{}][{}] = {}".format(ii, jj, noInject.H[ii, jj]))

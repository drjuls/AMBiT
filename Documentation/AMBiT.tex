...
AMBiT takes input from the command line and an input file. In principle, all parameters can be place in either. An exception is the directive to find the input file, which \emph{must} be placed in the command line using the -f option or by including a filename ending in .input, e.g.:
\begin{quote}
\begin{verbatim}
> ambit -f myFile.txt
> ambit -f "Carbon.inp"
...
\option{N} \textbf{[compulsory]} number of electrons included in HF procedure.
\option{Charge} can be used instead of HF/N: Charge = Z - N.
\option{Configuration} string indicating non-relativistic structure of HF core (potential including open shells), e.g.~\texttt{HF/Configuration = '1s2 2s2 2p6 3s2 3p6 3d4'}.

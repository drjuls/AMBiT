...
void LevelMap::Store(pHamiltonianID key, const LevelVector& level_vector)
{
    ...
    if(ProcessorRank == 0 && level_vector.NumLevels() && !filename_prefix.empty())
    {
        ...
        for(auto& pair: m_map)
        {
            ...
            if((pair.second.configs != nullptr) && pair.second.NumLevels())
                num_hamiltonians++;
        }
        ...
        for(auto& pair: m_map)
        {
            if((pair.second.configs != nullptr) && pair.second.NumLevels())
            {
                ...
                unsigned int num_levels = pair.second.NumLevels();
                file_err_handler->fwrite(&num_levels, sizeof(unsigned int), 1, fp);

                for(unsigned int i = 0; i < pair.second.NumLevels(); i++)
                {
                    const double* eigenvector = pair.second.eigenvectors.row(i).data();
                    unsigned int N = pair.second.configs->NumCSFs();
                    double eigenvalue = pair.second.eigenvalues[i];
                    double gfactor = std::numeric_limits<double>::quiet_NaN();
                    if(i < pair.second.g_factors.size())
                        gfactor = pair.second.g_factors[i];

                    file_err_handler->fwrite(&eigenvalue, sizeof(double), 1, fp);
                    file_err_handler->fwrite(&N, sizeof(unsigned int), 1, fp);
                    file_err_handler->fwrite(eigenvector, sizeof(double), N, fp);
...
void LevelMap::ReadLevelMap(pHamiltonianIDConst hamiltonian_example)
{
    ...
    for(unsigned int i = 0; i < num_hamiltonians; i++)
    {
        ...
        levelvec.Resize(num_levels, levelvec.configs->NumCSFs(), true);

        unsigned int N;

        for(unsigned int i = 0; i < num_levels; i++)
        {
            file_err_handler->fread(levelvec.eigenvalues.data()+i, sizeof(double), 1, fp);
            file_err_handler->fread(&N, sizeof(unsigned int), 1, fp);
            assert(N == levelvec.configs->NumCSFs());
            file_err_handler->fread(levelvec.eigenvectors.row(i).data(), sizeof(double), N, fp);
            file_err_handler->fread(levelvec.g_factors.data()+i, sizeof(double), 1, fp);

            // Check if we need to re-calculate this g-factor
            if(std::isnan(levelvec.g_factors[i]))
...
{
    keys.insert(key);
    if(ProcessorRank != 0 || level_vector.configs == nullptr || level_vector.NumLevels() == 0)
        return;
    ...
    unsigned int num_levels = level_vector.NumLevels();
    unsigned int N = level_vector.configs->NumCSFs();
    file_err_handler->fwrite(&num_levels, sizeof(unsigned int), 1, fp);

    for(unsigned int i = 0; i < num_levels; i++)
    {
        const double* eigenvector = level_vector.eigenvectors.row(i).data();
        double gfactor = std::numeric_limits<double>::quiet_NaN();
        if(i < level_vector.g_factors.size())
            gfactor = level_vector.g_factors[i];

        file_err_handler->fwrite(level_vector.eigenvalues.data()+i, sizeof(double), 1, fp);
        file_err_handler->fwrite(&N, sizeof(unsigned int), 1, fp);
        file_err_handler->fwrite(eigenvector, sizeof(double), N, fp);
        file_err_handler->fwrite(&gfactor, sizeof(double), 1, fp);
    }
